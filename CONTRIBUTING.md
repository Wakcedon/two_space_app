...
├── screens/                  # Экраны (UI)
├── services/                 # Бизнес-логика (Matrix, auth, chat)
...
test(services): добавить unit-тесты для MatrixService
```
...
flutter build appbundle --release (для Android)
```

Или используйте Git hook:
...
1. **Проверьте**, что бага нет в Issues
2. **Создайте новый Issue** с заголовком вида:
   ```
   [BUG] Краш приложения при отправке больших видео
   ```
3. **Опишите:**
   - Шаги для воспроизведения
   - Ожидаемое поведение
   - Актуальное поведение
   - Environment (ОС, версия Flutter, версия приложения)
   - Логи (если возможно)
